...
// 							$this->pi_lowerThan = ceil($this->internal['res_count']/$this->internal['results_at_a_time']);
	...
	function getSelectConf($where, $noPeriod = 0) {
		...
		if ($this->catExclusive && $this->config['categoryMode'] == 2) {
			...
			//$tmpCatExclusive = $this->actuallySelectedCategories;
...
// 		debug($selectConf,'select_conf');
	...
	/** 
	 * Create recursive path for categories
	 * @return array containing multidimensional array and path ineach position
	**/
	
	function generateCatPathRecursive($aId=0, $aPath=''){
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
				'uid , parent_category',
				'tt_news_cat',
				'parent_category ='.$aId);
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			$myPath = $aPath .','.$row['parent_category'];
			$retArray[$row['uid']]['recursive'] = $this->generateCatPathRecursive($row['uid'], $myPath);
			$retArray[$row['uid']]['path'] = explode(',',trim($myPath,','));
